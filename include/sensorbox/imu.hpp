...
private:
     ...
     *
     * This can be split into components where \f$ E \f$ is the Earth-Centered Earth-Fixed (ECEF) frame and \f$ N \f$ is
     * a chosen navigation frame (e.g. ENU or NED at the Earth's surface):
     * \f[
     ...
     * `frame()` \f$ F \f$ of the IMU, equal to \f$ \mathbf{a}^{F}_{IF} - \mathbf{g}^{F} \f$ where
     * \f$ \mathbf{a}^{F}_{IF} \f$ is the acceleration of the `frame()` with respect to an inertial reference frame
     * \f$ I \f$, expressed in `frame()`, and \f$ \mathbf{g}^{F} \f$ is the gravity expressed in `frame()`.
     *
     * One typically estimates for or knows \f$ \mathbf{g}^{N} \f$ where \f$ N \f$ is a navigation frame and we assume
     * small enough movement along this frame that \f$ \mathbf{g}^{N} \f$ does not change. Then:
     * \f[
     ...
     * frame (\f$ \mathbf{R}^{FN} = \mathbf{I} \f$) and at rest (\f$ \mathbf{a}^{N}_{IF} = \mathbf{0} \f$), then the
     * device will measure \f$ \mathbf{f}^F = -\mathbf{g}^{N} \f$. This yields an output \f$ [0, 0, +g] \f$ in ENU and
     * \f$ [0, 0, -g] \f$ in NED.
     *
     */
