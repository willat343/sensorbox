...
public:
     ...
     *
     * @param json json to check against schema
     * @param validate_ Calls validate(json) if true, otherwise does not validate, equivalent to default constructor.
     * May be set to false to prevent redundant validation checks in inheriting class structures where each class
     * inherits from a JsonLoadable (with extending schemas).
     */
    explicit JsonLoadable(const nlohmann::json& json, const bool validate_ = true);
